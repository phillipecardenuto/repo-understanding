...
| `dependency_path` | `source`, `target`, `max_paths` (1–10, default 3) | The shortest import chains from `source` to `target`, each step with its file, line and code. When there is none, it gives the chains in the other direction, if any. |
