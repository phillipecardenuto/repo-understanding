...
MIN_HOT_COMMITS = 2  # a file committed once is not churn
...
def _read_disk(root: Path, path: str) -> Any:
    """The working copy's bytes, ``None`` if absent (or reached through a symbolic link), or ``_TOO_LARGE``."""
    cur = root
    try:
        ...
        if not cur.is_file():
            return None
        if cur.stat().st_size > MAX_FILE_BYTES:
            return _TOO_LARGE
...
def _text(data: Any) -> tuple[str | None, str | None]:
    ...
    if data is _TOO_LARGE or (isinstance(data, bytes) and len(data) > MAX_FILE_BYTES):
...
def file_changes(repo: "Repository", path: str, commit: str | None = None, *, max_commits: int = MAX_COMMITS,
    ...
    head_blob = git.show_file(head, path) if head else None
    ...
    uncommitted = disk is _TOO_LARGE or disk != head_blob
    ...
    if shown == UNCOMMITTED:
        ...
        before = git.show_file(parent, path) if parent else None
        after = git.show_file(shown, path)
...
def hotspots(snapshot: Any, limit: int = REPORT_HOTSPOTS) -> list[str]:
    """The paths the Structure tab marks as churn hotspots (modules at or above the 80th percentile of
    commits, and changed at least twice), most changed first."""
    ...
    counts = sorted(churn.values())
    if not counts:
        return []
    hot = max(MIN_HOT_COMMITS, counts[int(len(counts) * 0.8)])
