...
| Agent review | `review.py` (targets, scope, signals, feedback prompt), `risk.py` (risk score), `contracts.py` (architecture contracts, baseline), `values.py` (constants and settings before → after, safety rules), `depchanges.py` (declared and locked dependency changes), `plan.py` (expected changes: plan vs actual, Markdown plan import) | `docs/review.md`, `docs/configuration.md` ("Architecture contracts") |
