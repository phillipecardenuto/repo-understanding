...
| Python | AST analyzer. Handles src/flat/namespace layouts, absolute, relative, `TYPE_CHECKING`, conditional, lazy and `importlib` imports, imports that rely on `sys.path` edits (read statically, marked *via sys.path*), symbols, call flow (aliases, re-exports, inheritance, nested functions), entry points and test functions. Optional grimp cross-check. |
...
| Manifests | `pyproject.toml` (PEP 621, Poetry, PDM, Hatch, setuptools, uv workspaces), `setup.py` (read with `ast`, never executed), `setup.cfg`, requirements (a `requirements.txt` next to a top-level package with no other manifest makes an *inferred* project), Pipfile, conda, `package.json` and npm/yarn/pnpm/lerna/rush workspaces, `tsconfig`, Deno, `Cargo.toml` and workspaces, `go.mod`/`go.work`, Maven, Gradle (+ settings), `.sln`/`.csproj`, Composer, Gemfile/gemspec, pubspec, mix, SwiftPM, CMake. Lock files are recognized. |
